...
namespace SimpleSAML\Module\ingameauth\Controller;

use SimpleSAML\Auth;
use SimpleSAML\Configuration;
use SimpleSAML\Error;
use SimpleSAML\HTTP\RunnableResponse;
use SimpleSAML\Module\ingameauth\Auth\Source\External;
...
/**
 * Controller class for the ingameauth module.
 *
...
class InGameAuth
{
    ...
    public function authpage(Request $request)
    {
         ...
         * administrator of the IdP leaves the ingameauth-module enabled in a production
         * environment.
         *
         * What we do here is to extract the $state-array identifier, and check that it belongs to
         * the ingameauth:External process.
         */
         ...
         */
        $this->authState::loadState(urldecode($matches[1]), 'ingameauth:External');
        ...
        $t = new Template($this->config, 'ingameauth:authenticate.twig');
    ...
    public function redirecttest(Request $request): RunnableResponse
    {
        ...
        /** @var array $state */
        $state = $this->authState::loadState($stateId, 'ingameauth:redirectfilter-test');
